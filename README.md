...
git config --list
git commit ~ julio
git commit é um comando utilizado no git. O git commit salva alterações feitas pelo git add no histórico do projeto.
...
principais caracteristicas -armazenamento de nuvem -controle de projeto na nuvem -controle de versões anteriores e atuais com o git -trabalho organizado e colaborativo
Bitbucket ~ Lucas
Descrição
